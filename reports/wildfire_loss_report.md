...
- **The future SSP1-2.6 scenario is illustrative only, not a forecast** — the projected +70% loss change reflects coefficients fit on 2 events and a feature-scale mismatch, not a reliable prediction of future risk (see Section 10).
...
Two dominant fuel codes (`101` and `31`, unlabelled — see Section 4) account for the majority of Alberta FSAs, consistent with the province's real agricultural/forested split and with the fuel-raster validation performed during pipeline development.

![Total insured value distribution](../outputs/figures/alberta_total_insured_value_distribution.png)

Total insured value per FSA (`Number of Exposure × $ Average Exposure`) is right-skewed, as expected given the individually right-skewed exposure count and average-exposure-value distributions in Section 5.1.
...
Poisson was the natural starting frequency model, but proved badly overdispersed, forcing a switch to Negative Binomial. Trying to use several FWI-family features together then ran into severe multicollinearity (they're almost collinear with each other — see Section 5.3), forcing a reduction to two low-collinearity climate predictors — and, since the assignment asks for both climate and geospatial features, one non-climate geospatial predictor (`dominant_fuel_pct`) was added on top. Only once the model itself was settled did validation (Section 8) and spatial dependence (Section 9) get layered on, followed by the future-climate projection (Section 10) as the last step, applying the finished model unchanged.
...
2. **Monte Carlo portfolio simulation.** Once enough events exist to identify a proper event-random-effect or copula structure, use it to simulate the portfolio loss distribution and produce real prediction intervals — not reported here (Section 10).
3. **Multi-model climate ensemble.** Widen the future projection beyond one model/scenario/member for a real uncertainty range, and resolve the event-window-vs-period-average feature scale mismatch (Section 10) before treating any future comparison as more than illustrative.
...
5. **Alternative severity predictors.** Reconsider whether severity should share the frequency model's climate predictors (Section 7), or whether an exposure-oriented predictor set fits better.
